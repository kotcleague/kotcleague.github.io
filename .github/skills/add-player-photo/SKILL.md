...
Use this skill when the user provides a player name and a direct photo URL and
requests a local copy.
...
The command accepts HTTP(S) URLs, validates the response as a supported image
type, derives the filename from the existing player ID, and updates every
ranking view for that player. Scraped URLs remain external by default; use this
skill only when an image host blocks hotlinking or the user explicitly wants a
local copy.
